...
WHICH_DATA     = all_data
STOCH_THRESH   = 20
OUTPUT_PATH    = str(input('what do you want to name this strat?'))
SHORT_STRATS   = False
UP_PCT_TARGETS =  [6,8,11,13]
DN_PCT_TARGETS =  [1,3,4,6]
...
for col in all_data:#daily_data:
    ...
    if SHORT_STRATS == True:
        print('old close',df.close[1])
        short_frame(df)
        print('new_close',df.close[1])
    df
    ...
    for candle in canli:
        ...
        phdf.to_csv(grid_path+OUTPUT_PATH+':'+grid_name)
